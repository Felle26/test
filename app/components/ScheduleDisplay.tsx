...
export default function ScheduleDisplay({ weekId, availableWeeks, isManagementView, startEditing, screenSaverMinutes, isModified, company, branch, period, days, employees, roles }: ScheduleDisplayProps) {
  ...
  const renderShiftCell = (shift: ScheduleEmployee["shifts"][number]) => {
    ...
    return <div className="rounded border border-slate-300 p-2 text-sm font-bold leading-snug" style={{ color: shift.textColor, backgroundColor: shift.backgroundColor }}>{shift.content.split("\n").map((line, index) => <div key={index}>{line}</div>)}</div>;
  };
  ...
  return <div className={`min-h-screen w-full select-none touch-manipulation px-3 py-3 transition-colors duration-200 sm:px-6 sm:py-6 ${isDarkMode ? "bg-slate-950" : "bg-slate-100"}`}>
    <ScheduleScreenSaver enabled={!isManagementView && !isEditingEnabled && !selectedCell} idleMinutes={screenSaverMinutes} />
    <section className={`mx-auto w-full max-w-[1920px] overflow-hidden rounded-xl border shadow-md ${isDarkMode ? "border-slate-700 bg-slate-950" : "border-slate-200 bg-white"}`}>
    ...
    <div className="max-h-[calc(100vh-13rem)] overflow-x-auto overflow-y-auto"><table className="w-full border-collapse text-left"><thead className={`sticky top-0 z-10 shadow-sm ${isDarkMode ? "bg-slate-900" : "bg-slate-100"}`}><tr className={`border-b text-base ${isDarkMode ? "border-slate-700 text-slate-200" : "text-slate-700"}`}><th className="min-w-48 p-4 font-semibold">Mitarbeiter</th>{days.map((day, index) => <th key={index} className={`min-w-36 border-l p-4 text-center font-semibold ${isDarkMode ? "border-slate-700" : ""}`}><div>{day.title}</div><div className={isDarkMode ? "text-sm font-normal text-slate-400" : "text-sm font-normal text-slate-500"}>{day.date}</div></th>)}</tr></thead><tbody className={`text-base ${isDarkMode ? "divide-y divide-slate-800" : "divide-y divide-gray-200"}`}>{visibleEmployees.map((employee, employeeIndex) => <tr key={`${employee.name}-${employee.nr}-${employeeIndex}`} className={`transition ${isDarkMode ? "hover:bg-slate-900" : "hover:bg-slate-50"}`}><td className={isDarkMode ? "p-4 font-medium text-slate-100" : "p-4 font-medium text-slate-900"}>{employee.name}</td>{days.map((_, dayIndex) => <td key={dayIndex} className={`border-l p-2 text-center align-middle ${isDarkMode ? "border-slate-800" : ""}`}><button type="button" disabled={!isEditingEnabled} onClick={() => openEditor(employee, dayIndex)} className={`block min-h-16 w-full touch-manipulation rounded p-1 text-left transition focus:outline-2 focus:outline-offset-1 focus:outline-emerald-500 disabled:cursor-not-allowed disabled:opacity-100 ${isEditingEnabled ? isDarkMode ? "hover:bg-slate-800" : "hover:bg-slate-100" : ""}`} aria-label={`Schicht für ${employee.name} am ${days[dayIndex]?.title ?? "Tag"} bearbeiten`} title={isEditingEnabled ? "Dienstplaneintrag bearbeiten" : "Bearbeitung zuerst global freigeben"}>{renderShiftCell(employee.shifts[dayIndex])}</button></td>)}</tr>)}</tbody></table>{visibleEmployees.length === 0 && <p className={isDarkMode ? "p-8 text-center text-base text-slate-400" : "p-8 text-center text-base text-slate-500"}>Keine Mitarbeiter für diesen Filter gefunden.</p>}</div>
    </section>
